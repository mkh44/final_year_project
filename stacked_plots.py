...
for asdf_file in asdf_files:
    iris_window_underscore = os.path.basename(asdf_file).replace('IRIS_fitting_', '').replace(event, '').replace(
        '.asdf', '')
    ...
    plot_iris_sns_fits(int_map, dopp_map, width_map, vnt_map, asym_map, iris_window, event, main_header, asym_rng,
                       dopp_rng, max_wid, max_vnt)

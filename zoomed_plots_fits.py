...
def plot_iris_sns_fits(int_map, dopp_map, width_map, vnt_map, asym_map, iris_window, event, main_header, asym_rng,
                       dopp_rng, max_wid, max_vnt):
    ...
    plt.savefig(os.path.join(output_loc, event) + f"/IRIS_zoomed_plot_" + iris_window.replace(' ',
                                                                                              '_') + '_' + file_time + '_' + f"{t_start:.0f}-{t_end:.0f}s_from_raster_start.png",
                bbox_inches='tight')
    plt.close(fig)
...
fits_files = glob.glob(os.path.join(output_loc, "*.fits"))

if not fits_files:
    raise FileNotFoundError(f"No FITS files found in {output_loc}")

# Get map names
# ----------------------
for fits_file in fits_files:
    #iris_window_underscore = os.path.basename(fits_file).replace('IRIS_fitting_', '').replace(event, '').replace(
    #    '.fits', '')
   # iris_window = iris_window_underscore.replace('_', ' ').strip()

    #print(iris_window)
    opf = fits.open(fits_file)
    opf.info()
    int_map = sunpy.map.Map(opf['int'])
    dopp_map = sunpy.map.Map(opf['dopp'])
    width_map = sunpy.map.Map(opf['width'])
    asym_map = sunpy.map.Map(opf['asym'])

    if 'vnt' in opf:
        vnt_map = sunpy.map.Map(opf['vnt'])
    else:
        vnt_map = None
    opf.close()

    # possible_int_keys = [key for key in opf.keys() if 'int' in key.lower()]
    ...
    plot_y_for_time_series(int_map, dopp_map, width_map, vnt_map, asym_map, iris_window, event, main_header,
                           y_value_arcsec=235)

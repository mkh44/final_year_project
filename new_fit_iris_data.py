...
def plot_iris_fits(int_map, dopp_map, width_map, vnt_map, asym_map, aspect_ratio, iris_window, event):
    ...
    plt.savefig(output_loc + event + '/IRIS_analysis_' + iris_window.replace(' ', '_') + '_' + file_time + '.png',
                bbox_inches='tight')
    plt.close(fig)


# Plot the output of the fitting routine
def plot_iris_quartile_fits(int_map, dopp_map, width_map, asym_map, aspect_ratio, iris_window, event):
    fig = plt.figure(constrained_layout=True, figsize=(10, 6))
    plt.rcParams['font.size'] = '10'

    gs = GridSpec(nrows=1, ncols=4, hspace=0.1, wspace=0.1)
    gs.update(left=0.05, right=0.95, bottom=0.04, top=0.99)
...
def plot_iris_sns_fits(int_map, dopp_map, width_map, vnt_map, asym_map, iris_window, event, main_header):
    ...
    plt.imshow(vnt_map.data, norm=norm, cmap=mpl.colormaps['inferno'], axes=ax5, origin='lower',
               extent=[t_array.min(), t_array.max(), slit_pos.min(), slit_pos.max()], aspect='auto')
    ax5.set_ylabel(" ")
...
def plot_mgii_fits(dv_k3_map, dv_h3_map, k2_sep_map, h2_sep_map, aspect_ratio, iris_window, event):
    ...
    plot_time = dt.datetime.strftime(dt.datetime.strptime(dv_k3_map.meta['date-obs'], '%Y-%m-%dT%H:%M:%S.%f'),
                                     '%Y/%m/%dT%H:%M:%S')
    file_time = dt.datetime.strftime(dt.datetime.strptime(dv_k3_map.meta['date-obs'], '%Y-%m-%dT%H:%M:%S.%f'),
                                     '%Y%m%d_%H%M%S')
...
def plot_mgii_sns_fits(dv_k3_map, dv_h3_map, k2_sep_map, h2_sep_map, iris_window, event, main_header):
    ...
    slit_pos = dv_k3_map.meta['crval2'] + dv_k3_map.meta['cdelt2'] * (
                np.arange(dv_k3_map.data.shape[0]) - dv_k3_map.meta['crpix2'])
...
def plot_mgii_quartiles(mgii_k_integ_int, mgii_h_integ_int, mgii_k_vdopp, mgii_h_vdopp, mgii_k_width, mgii_h_width,
                        mgii_k_asym, mgii_h_asym, mg_integ_int, aspect_ratio, iris_window, event):
    ...
    plot_time = dt.datetime.strftime(dt.datetime.strptime(mgii_k_vdopp.meta['date-obs'], '%Y-%m-%dT%H:%M:%S.%f'),
                                     '%Y/%m/%dT%H:%M:%S')
    file_time = dt.datetime.strftime(dt.datetime.strptime(mgii_k_vdopp.meta['date-obs'], '%Y-%m-%dT%H:%M:%S.%f'),
                                     '%Y%m%d_%H%M%S')
...
def plot_mgii_sns_quartiles(mgii_k_integ_int, mgii_h_integ_int, mgii_k_vdopp, mgii_h_vdopp, mgii_k_width, mgii_h_width,
                            mgii_k_asym, mgii_h_asym, mg_integ_int, iris_window, event, main_header):
    ...
    slit_pos = mgii_k_vdopp.meta['crval2'] + mgii_k_vdopp.meta['cdelt2'] * (
                np.arange(mgii_k_vdopp.data.shape[0]) - mgii_k_vdopp.meta['crpix2'])
    pdb.set_trace()
    dopp_rng = 30
    max_wid = 1.0
    asym_rng = 1
    ...
    norm = colors.Normalize(vmin=0, vmax=upr_bnd)
    plt.imshow(mgii_k_integ_int.data, norm=norm, cmap=mpl.colormaps['Reds_r'], axes=ax00, origin='lower',
               extent=[t_array.min(), t_array.max(), slit_pos.min(), slit_pos.max()], aspect='auto')
    ax00.set_ylabel("Solar Y (arcsec)")
    ax00.set_xlabel(" ")
    ...
    norm = colors.Normalize(vmin=-dopp_rng, vmax=dopp_rng)
    plt.imshow(mgii_k_vdopp.data, norm=norm, cmap=mpl.colormaps['seismic'], axes=ax01, origin='lower',
               extent=[t_array.min(), t_array.max(), slit_pos.min(), slit_pos.max()], aspect='auto')
    ax01.set_ylabel(" ")
    ax01.set_xlabel(" ")
    ...
    norm = colors.Normalize(vmin=0, vmax=max_wid)
    plt.imshow(mgii_k_width.data, norm=norm, cmap=mpl.colormaps['cubehelix'], axes=ax02, origin='lower',
               extent=[t_array.min(), t_array.max(), slit_pos.min(), slit_pos.max()], aspect='auto')
    ax02.set_ylabel(" ")
    ax02.set_xlabel(" ")
    ...
    norm = colors.Normalize(vmin=-asym_rng, vmax=asym_rng)
    plt.imshow(mgii_k_asym.data, norm=norm, cmap=mpl.colormaps['coolwarm'], axes=ax10, origin='lower',
               extent=[t_array.min(), t_array.max(), slit_pos.min(), slit_pos.max()], aspect='auto')
    ax10.set_ylabel("Solar Y (arcsec)")
    ax10.set_xlabel(" ")
    ...
    norm = colors.Normalize(vmin=0, vmax=upr_bnd)
    plt.imshow(mgii_h_integ_int.data, norm=norm, cmap=mpl.colormaps['Reds_r'], axes=ax11, origin='lower',
               extent=[t_array.min(), t_array.max(), slit_pos.min(), slit_pos.max()], aspect='auto')
    ax11.set_ylabel(" ")
    ax11.set_xlabel(" ")
    ...
    norm = colors.Normalize(vmin=-dopp_rng, vmax=dopp_rng)
    plt.imshow(mgii_h_vdopp.data, norm=norm, cmap=mpl.colormaps['seismic'], axes=ax12, origin='lower',
               extent=[t_array.min(), t_array.max(), slit_pos.min(), slit_pos.max()], aspect='auto')
    ax12.set_ylabel(" ")
    ax12.set_xlabel(" ")
    ...
    norm = colors.Normalize(vmin=0, vmax=max_wid)
    plt.imshow(mgii_h_width.data, norm=norm, cmap=mpl.colormaps['cubehelix'], axes=ax20, origin='lower',
               extent=[t_array.min(), t_array.max(), slit_pos.min(), slit_pos.max()], aspect='auto')
    ax20.set_ylabel("Solar Y (arcsec)")
    ...
    norm = colors.Normalize(vmin=-asym_rng, vmax=asym_rng)
    plt.imshow(mgii_h_asym.data, norm=norm, cmap=mpl.colormaps['coolwarm'], axes=ax21, origin='lower',
               extent=[t_array.min(), t_array.max(), slit_pos.min(), slit_pos.max()], aspect='auto')
    ax21.set_ylabel(" ")
    ...
    norm = colors.Normalize(vmin=min_rat, vmax=max_rat)
    plt.imshow(mg_integ_int.data, norm=norm, cmap=mpl.colormaps['cubehelix_r'], axes=ax22, origin='lower',
               extent=[t_array.min(), t_array.max(), slit_pos.min(), slit_pos.max()], aspect='auto')
...
# Fit the Si IV & C II spectral lines
def fit_iris(file, iris_window, event, do_fit=False):
    ...
    if do_fit:
        ...
        line_pos = quartiles[:, :, int_50]
        integ_int = quartiles[:, :, 3]

        if header['CDELT3'] == 0:
            q_int_map = a.mk_iris_map(integ_int.T, header, main_header)
        else:
            q_int_map = a.mk_iris_map(integ_int, header, main_header)

        # Doppler velocity
        # We're going to take the average median of the datacube as the rest wavelength in the region of interest
        wvl_ref = np.nanmean(line_pos)
        ref_wave = np.full((y_size, x_size), wvl_ref)
        v_dopp = ((line_pos - ref_wave) / (ref_wave)) * (speed_of_light / 1e3)
        if header['CDELT3'] == 0:
            q_dopp_map = a.mk_iris_map(v_dopp.T, header, main_header)
        else:
            q_dopp_map = a.mk_iris_map(v_dopp, header, main_header)

        # Line width
        line_width = quartiles[:, :, int_75] - quartiles[:, :, int_25]
        if header['CDELT3'] == 0:
            q_width_map = a.mk_iris_map(line_width.T, header, main_header)
        else:
            q_width_map = a.mk_iris_map(line_width, header, main_header)

        # Asymmetry
        asym = (((quartiles[:, :, int_75] - quartiles[:, :, int_50]) -
                 (quartiles[:, :, int_50] - quartiles[:, :, int_25])) / (
                            quartiles[:, :, int_75] - quartiles[:, :, int_25]))
        if header['CDELT3'] == 0:
            q_asym_map = a.mk_iris_map(asym.T, header, main_header)
        else:
        ...
        with asdf.open(
            ...
            q_int_map = af.tree['q_int_map']
            q_dopp_map = af.tree['q_dopp_map']
            q_width_map = af.tree['q_width_map']
            q_asym_map = af.tree['q_asym_map']
    ...
    if int_map.meta['cdelt1'] == 1e-9:
        ...
        plot_iris_sns_quartile_fits(q_int_map, q_dopp_map, q_width_map, q_asym_map, save_window, event, main_header)
    else:
        ...
        plot_iris_quartile_fits(q_int_map, q_dopp_map, q_width_map, q_asym_map, aspect_ratio, save_window, event)

    return


# Get the Mg II line properties
def fit_iris_mgii(file, iris_window, event, plot_time, do_fit=False):
    int_25 = 0
    int_50 = 1
    int_75 = 2
    ...
    if do_fit:
        ...
        quartiles_k = get_quartiles.get_quartiles(file, 'Mg II k', iris_window)

        y_size = quartiles_k.shape[0]
        x_size = quartiles_k.shape[1]
        line_pos = quartiles_k[:, :, int_50]

        integ_int = quartiles_k[:, :, 3]
        if header['CDELT3'] == 0:
        ...
        asym = (((quartiles_k[:, :, int_75] - quartiles_k[:, :, int_50]) -
                 (quartiles_k[:, :, int_50] - quartiles_k[:, :, int_25])) / (
                            quartiles_k[:, :, int_75] - quartiles_k[:, :, int_25]))
        if header['CDELT3'] == 0:
        ...
        quartiles_h = get_quartiles.get_quartiles(file, 'Mg II h', iris_window)

        y_size = quartiles_h.shape[0]
        x_size = quartiles_h.shape[1]
        line_pos = quartiles_h[:, :, int_50]

        integ_int = quartiles_h[:, :, 3]
        if header['CDELT3'] == 0:
        ...
        asym = (((quartiles_h[:, :, int_75] - quartiles_h[:, :, int_50]) -
                 (quartiles_h[:, :, int_50] - quartiles_h[:, :, int_25])) / (
                            quartiles_h[:, :, int_75] - quartiles_h[:, :, int_25]))
        if header['CDELT3'] == 0:
            mgii_h_asym = a.mk_iris_map(asym.T, header, main_header)
        else:
            mgii_h_asym = a.mk_iris_map(asym, header, main_header)
        # Integrated intensity
        int_ratio = np.divide(quartiles_k[:, :, 3], quartiles_h[:, :, 3])
        if header['CDELT3'] == 0:
            mg_int_rat = a.mk_iris_map(int_ratio.T, header, main_header)
        else:
        ...
        tree = {'dv_k3_map': dv_k3_map, 'dv_h3_map': dv_h3_map, 'k2_sep_map': k2_sep_map, 'h2_sep_map': h2_sep_map,
                'mgii_k_integ_int': mgii_k_integ_int, 'mgii_h_integ_int': mgii_h_integ_int,
                'mgii_k_vdopp': mgii_k_vdopp,
...
def fitdata(event):
    ...
    for indiv_file in f_iris_raster:
        ...
        file_time = dt.datetime.strftime(dt.datetime.strptime(main_header['DATE_OBS'], '%Y-%m-%dT%H:%M:%S.%f'),
                                         '%Y%m%d_%H%M%S')
        ...
        if files == True:
            ...
            print("Updating plot...")
            fit_iris_mgii(indiv_file, 'Mg II k 2796', event, file_time, do_fit=False)
        if files == False:
            print('Fitting Mg II data')
            fit_iris_mgii(indiv_file, 'Mg II k 2796', event, file_time, do_fit=True)
